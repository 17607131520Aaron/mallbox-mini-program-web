...
export const initGlobalClientErrorReporting = (): (() => void) | undefined => {
  ...
  const handleRuntimeError = async (event: ErrorEvent): Promise<void> => {
  ...
  const handleUnhandledRejection = async (event: PromiseRejectionEvent): Promise<void> => {
 ...
 */
export const withClientErrorGuard = async <T extends (...args: unknown[]) => unknown>(
  actionName: string,
  fn: T,
): Promise<T> =>
